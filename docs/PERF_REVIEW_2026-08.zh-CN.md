...
| B4 | `/export` worker 按 `_cache_identity` 复用 npz 里已持久化的全分辨率 Analysis | GUI 导出 −1.5~2.5s | 待做 |
...
| E12b | 导出抖动平面复用（preview 已有确定性平面）；`_clip_masks_resized` 双拷贝；`y/ev` 死重 | −0.3s −400MB | 待做 |
| E12c | matplotlib 迟滞到出图时；JPEG 直接走 PIL | `import dngscan` 0.42→0.15s（每个 spawn worker 均摊） | ✅ 1bfa02b |
| E18 | SDR 渲染池 2 worker → 与 HDR 侧统一定容 | pre-tone NumPy 段并行度翻倍 | 待做 |
