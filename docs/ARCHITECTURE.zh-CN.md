...
- **显影曲线 + 观看条件翻译**（二层）：端到端目标由**光谱印相**构造——负片染料
  堆栈的透过谱 × 相纸自己的光谱感度 × 3200K 放大机（TH-KG3 约定）→ 相纸逐染料
  显影 → 印片光谱在声明观看光源下做相对比色——再最小二乘解到 AgX 参数空间的
  一个具名坐标；预设激活时整卷一致、场景自适应关闭，EV0→0.18 由全局曝光锚
  （测光表语义）保证，介质地板经 `target_black_linear` 进入曲线。**翻译是观看
  条件完整的**：观看闪光按介质分派（反射印片 1.0%，IEC 61966-2-1；暗环境投影
  为 0），暗环境介质（反转片、Vision3→2383 放映链）统一按经典 surround 常数
  （dark 1.5 / dim 1.2 / average 1.0，Bartleson-Breneman，Fairchild 2013）折算到
  明环境交付；想要"影院拷贝原样上监视器"的是 `*_theatrical` 引用变体——合同
  区分**翻译**与**引用原文**，引用不占用胶片名，也不带观看翻译。
- **逐通道比率场**（仅 full 模式）：从同一特性曲线解出的层饱和差异
  `r_c(EV) = T_c/T_neutral`，按 `r_c(EV_c)/r_c(EV_Y)` 施加（中性轴恒 1，构造
  保证）。它是"胶片接管显影侧颜色"的载体——而色彩维度的重建**没有外部
  oracle**（音调维度有），因此默认关闭。

**双模式分工**（`--film-mode`，合同 §光谱印相·双模式）：**observe（默认）**=
胶片声明观察者看见了什么（WB/分离/音调签名），颜色由 AgX 显影——分工画在
数据可信边界上，胶片预设在此模式下只是曲线参数，原生内核照常加速；
**full（实验）**= 胶片显影模型整体接管（`film_develop` 核：逐通道曲线×比率场，
无 AgX 色彩几何），AgX 只保留交付端色域安全，暂仅 SDR。关闭 `--film` 即纯
AgX。逐通道滚降本身就是颜色操作——这就是"AgX 只管拉伸滚降、颜色全给胶片"
...
当前拟合水平（光谱印相后）：25 支全部 rms ≤0.061——负片系 0.012–0.033、
反转片系 0.028–0.053、theatrical 引用系 0.026–0.061；黑端域外钉界全部消失
（闪光地板给了趾部真实落点，black_ev 成为数据决定）。

### 外部验证

`tools/crosscheck_2383.py` 用独立实现（DiVERE 的 Kodak 2383 曲线，曲线域约定从其
源码逐行核对）交叉验证密度域合成——这把外部尺子用了两次：第一轮定位了旧通道
捷径的缺陷（B 通道 rms 0.383，尺度阶梯 0.75/0.84/0.96 随波长单调——印片有效
密度被误读的光谱签名）；光谱印相升级后第二轮验证通过（阶梯消失，B 塌到
0.048，三通道全部 ≤0.086 stop 合流）。详见合同·外部互证与光谱印相两节。
