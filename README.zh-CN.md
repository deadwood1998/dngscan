...
- **因此固定 `--ev` 并不足以隔离解码器差异。** 窗口更宽意味着 Core Image 路径编译出的
  ...
  部分，有一些来自这里而不是来自解码。`tools/decode_ab.py` 把两者分开：让每个缓冲各自
  走过每套计划，再做归因。它是诊断工具，而且**故意会出难看的图**——用一个缓冲编译的计划
  去套另一个缓冲，端点必然落错位置。某张 ISO 12800 片子上它显示原生对比掩盖的和揭示的
  一样多：纯白像素的原生差异是 0.00%，但 Core Image 缓冲单独贡献 +1.49%，而它自己更宽的
  计划恰好抵消了这么多。
