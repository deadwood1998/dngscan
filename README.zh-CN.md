...
**已知问题：RAW 9 的爆光高光偏品红。** 在高光被剪切的地方，Apple 返回的缓冲里绿通道被
钉死在远低于红蓝的位置——某张片子受影响像素的均值为 R 2.02 / G 0.73 / B 1.94，且这些
像素里绿为最大通道的占比是 0.0%。结果是品红色的高光核心和粉色光晕：该片近白像素实测
品红偏移 +0.080（R/G 1.043、B/G 1.043），而 LibRaw 路径在同一处中性到 0.005 以内。它在
视图变换之前就存在，所以是解码属性而非 AgX 产物，AgX 的高光褪色只能冲淡一部分。LibRaw
路径不出现是因为 `--highlight-mode clip` 会把所有通道剪到同一白点——代价是丢掉了 Apple
保留下来的高光滚降。所测的每一张 Sigma fp 片子都有这个签名（占 0.3% 到 1.5% 的像素），
因此应按系统性问题看待。要修复就得从解码后的缓冲反推剪切位置，而那正是这条管线有意
放弃的逐像素 CFA 证据；目前的处理是记录而非纠正。
