...
dngscan 会清零所有暴露出来的控制项——包括默认 0.485、在版本 8 上无效而在版本 9 上生效的
`sharpnessAmount`，以及在版本 9 上 `isSupported` 报 false、但 0.5 的默认值仍影响 93.6%
像素的 `colorNoiseReductionAmount`——即便如此，残余差异依然很大，而且差多少取决于场景。
以画面最暗 30% 区域内 8×8 块局部标准差的中位数为度量，两条路径都固定 `--ev 0`：

| 片子 | ISO | 亮度噪声 / LibRaw | 色度噪声 / LibRaw | 纯黑像素 |
| --- | --- | --- | --- | --- |
| 演出，近乎全黑 | 25600 | 12% | 11% | 21.6% 对 8.1% |
| 园林，阴天日光 | 12800 | 81% | 29% | 2.8% 对 1.4% |

色噪的清理是稳定的，亮噪不是。模型的优势主要来自信噪比真正糟糕的地方；在曝光正常的
片子上，最暗的 30% 只是**影调**暗而并不缺信号，两条路径于是几乎收敛。两张片都以压暗
阴影为代价。把 LibRaw 换成更平滑的去马赛克（VNG、PPG）并不能缩小差距，所以这是模型本身
而非插值选择。这一点与本工具"不做降噪、把纹理选择留给去马赛克"的立场需要各自权衡。

**清零控制项的边界在"重建"处。**"把控制项全部清零"是从 LibRaw 路径继承来的规矩，把它
不加分辨地套到一个假设完全不同的解码器上，得到的不是更纯净的解码，而是更错误的解码。
因此有两项 CIRAWFilter 设置保持 Apple 的取值，各有实测依据：
...
另有两项 Apple 的默认值是**有意覆盖**而非接受的。`boostAmount` 与 `boostShadowAmount`
默认为 1.0 和 0.9，那是 Apple 的相机 look 而非线性转换，二者都清零。`baselineExposure`
默认取 DNG 文件里的值（这些片子上是 1.0，等于给每个像素 +1 EV），强制为 0——曝光是
dngscan 通过 `--ev`、auto-EV 和中灰余量模型自己掌管的轴。`extendedDynamicRangeAmount`
保持 Apple 默认的 0：调到 1.0 确实在最顶端拉出更多分离度（顶部像素极差 0.11 → 1.74），
但高光区与默认渲染在 log2 上的相关系数是 0.996，说明基本是同一批信息的重映射，而且会把
峰值推到 20，远超这条管线预留的余量。
