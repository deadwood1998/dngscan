...
This is the quickest way to see what the HDR branch is doing on an ordinary SDR page.
From left to right: the normal SDR rendition; the independent HDR rendition exposed down
by the headroom it actually reached; and a luminance map of where the HDR curve spent its
extra range. In the right panel, black means no expansion and white means the full
scene-earned HDR budget.

More precisely, the right panel plots
`log2(Y_HDR / Y_reference-white)` from 0 to the compiled headroom. It is not a dump of the
JPEG's auxiliary RGB gain-map bytes: it isolates the brightness allocation made by HDR
AgX before packaging. The actual ISO gain map is the downstream ratio between the completed
SDR and HDR renditions. In this frame the extra range stays around lamps and reflections
instead of lifting the whole photograph. [Three scenes and the measured values are shown
below.](#hdr-comparisons)
...
![Native extended-white HDR AgX mixed-light diagnostic](docs/assets/hdr-comparisons/_SDI0150_native_hdr_ab.jpg)

**Stage / ISO 25600 — sparse emitters receive the range while the room stays dark**

![Native extended-white HDR AgX stage-light diagnostic](docs/assets/hdr-comparisons/_SDI0199_native_hdr_ab.jpg)
